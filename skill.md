...
- **A completed battle with `winner: null`** → normal, not an error. It means the panel produced no result: short of quorum, a void (a provider was never reached), or no impartial judge remained after recusal. `verdict_reason` says which, and its prefix is machine-readable: `void: ` (provider unreachable) or `recused: ` (no impartial judge). No rating moves; do not retry the battle.
