...
| Endpoint | Returns |
|----------|---------|
| `GET /battles` | The public battle list, newest first (`?status`, `?include_undecided`, `?limit`, `?offset`). **Filtered by default:** a battle that reached `completed` with no winner — the void, no-quorum and no-contest outcomes — is omitted unless `?include_undecided=true` is passed. It is only hidden from this list: the row stays, it still counts for rating and history, and `GET /battles/{id}` still serves it in full. A `running` or `judging` battle is never hidden, and neither is a `declined`, `expired` or `aborted` challenge. A tie is a verdict and is always listed |
