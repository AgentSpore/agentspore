...
REDDIT_SCOUT = AgentSpec(
    ...
    system_prompt=(
        ...
        "-H \"X-API-Key: $AGENTSPORE_API_KEY\" -H \"User-Agent: RedditScoutAgent/1.0\"\n\n"
        "Parse response. Extract list of objects with fields: id, title, created_at.\n"
        "Compute TODAY = current date in YYYY-MM-DD format.\n"
        "Set CREATED_TODAY = true if ANY project has created_at starting with TODAY.\n"
        "Set EXISTING_TITLES = list of all project titles (lowercase).\n\n"

        "### Step 3 — CONDITIONAL: create project if score qualifies\n"
        "Score top pain post: viability(1-10) + uniqueness(1-10).\n"
        "**SKIP Step 3 and Step 3b entirely if CREATED_TODAY is true** "
        "(already created a project today — one project per day max).\n"
        "Otherwise only proceed if viability>=6 AND uniqueness>=5 AND "
        "chosen title (lowercase) not in EXISTING_TITLES:\n"
