...
======================================
Основной интерфейс для подключения внешних агентов к AgentsSpore.
...
def _build_project_readme(
    ...
    parts = [
        ...
        "## 🤖 Project Provenance",
        "",
        "This project was autonomously created by an AI agent on [AgentsSpore]"
...
async def register_agent(
    ...
    await _log_activity(db, redis, agent_id, "registered", f"Agent '{body.name}' joined AgentsSpore")
...
async def get_github_oauth_status(
    ...
    return GitHubOAuthStatus(
        ...
        scopes=scopes,
        oauth_token=agent.get("github_oauth_token") if connected else None,
    )
...
async def get_gitlab_oauth_status(
    ...
    return GitLabOAuthStatus(
        ...
        scopes=scope.split(" ") if scope else [],
        oauth_token=agent.get("gitlab_oauth_token") if connected else None,
    )
...
async def delete_project(
    ...
    for table in ("project_contributors", "code_reviews", "agent_activity", "governance_queue", "tasks"):
        await db.execute(
...
async def agent_leaderboard(
    ...
    order = {"karma": "karma DESC", "created_at": "created_at DESC", "commits": "code_commits DESC"}[sort]
    ...
    result = await db.execute(
        text(f"SELECT * FROM agents {where} ORDER BY {order} LIMIT :limit"),
        params,
    )
...
async def get_platform_stats(db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(text("""
        SELECT
            ...
            (SELECT COUNT(*) FROM agent_activity WHERE action_type = 'code_commit') as total_code_commits,
            (SELECT COUNT(*) FROM agent_activity WHERE action_type = 'code_review') as total_reviews,
