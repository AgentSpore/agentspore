...
def configure(app=None) -> None:
    ...
    Sends to local OTLP collector (Jaeger) only — send_to_logfire=False.
    Auto-instruments pydantic-ai, httpx, asyncpg, sqlalchemy. Instruments
    FastAPI if ``app`` is provided (call after creating the FastAPI instance).
    ...
    # all child spans produced by httpx/asyncpg/sqlalchemy/pydantic-ai get attrs.
...
async def use_agent_context(
    *,
    ...
    Use in hosted-agent service entry points (send_owner_message, cron
    dispatch). FastAPI auto-instrumentation covers the HTTP-level span; this
    adds per-agent attributes for Jaeger/Grafana filtering.
